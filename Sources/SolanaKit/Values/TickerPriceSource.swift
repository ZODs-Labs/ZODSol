...
/// mints are quoted from `jupiter`. Arbitrary EVM tokens (Ethereum, Base, BSC
/// and other supported chains) are quoted from `evmDex`, a keyless on-chain DEX
/// indexer. The source is resolved once when a token is added and frozen onto
/// the entry; the refresh loop never re-derives it.
