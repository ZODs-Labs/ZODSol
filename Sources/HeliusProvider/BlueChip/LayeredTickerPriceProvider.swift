...
/// to the keyless CEX client (Kraken, Coinbase fallback) for real spot, and
/// arbitrary Solana mints to Jupiter. The two source fetches run concurrently
/// and merge into one keyed-by-identifier outcome, so the engine stays
/// source-agnostic. Kraken drives the backoff signal; Coinbase only adds quotes.
public struct LayeredTickerPriceProvider: TickerQuoteProviding {
    ...
    public func quotes(for requests: [TickerQuoteRequest]) async -> TickerFetchOutcome {
        ...
        let (kraken, jupiterResult) = await (krakenOutcome, jupiterOutcome)

        var quotes = kraken.quotes
        quotes.merge(jupiterResult.quotes) { _, new in new }
        let retryAfter = [kraken.retryAfter, jupiterResult.retryAfter].compactMap(\.self).max()
        return TickerFetchOutcome(
            quotes: quotes,
            retryAfter: retryAfter,
            shouldBackOff: kraken.shouldBackOff || jupiterResult.shouldBackOff)
    }
