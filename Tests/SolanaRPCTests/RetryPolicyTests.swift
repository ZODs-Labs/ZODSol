...
final class RetryPolicyTests: XCTestCase {
    ...
    func test_delay_attempt1_inJitterBandAround1s() {
        ...
        for _ in 0..<200 {
            let delay = self.policy.delay(for: 1, retryAfter: nil)
            let seconds = self.durationSeconds(delay)
    ...
    func test_delay_attempt2_inJitterBandAround2s() {
        for _ in 0..<200 {
            let delay = self.policy.delay(for: 2, retryAfter: nil)
            let seconds = self.durationSeconds(delay)
    ...
    func test_delay_attempt3_inJitterBandAround4s() {
        for _ in 0..<200 {
            let delay = self.policy.delay(for: 3, retryAfter: nil)
            let seconds = self.durationSeconds(delay)
    ...
    func test_retryAfter_nonNil_returnsRetryAfter() {
        let delay = self.policy.delay(for: 1, retryAfter: .seconds(7))
        XCTAssertEqual(self.durationSeconds(delay), 7.0, accuracy: 0.0001)
    }

    func test_retryAfter_largerThanMaxDelay_isCapped() {
        let delay = self.policy.delay(for: 1, retryAfter: .seconds(120))
        XCTAssertEqual(self.durationSeconds(delay), 30.0, accuracy: 0.0001)
    }

    func test_retryAfter_atExactMaxDelay_isReturnedUnchanged() {
        let delay = self.policy.delay(for: 1, retryAfter: .seconds(30))
        XCTAssertEqual(self.durationSeconds(delay), 30.0, accuracy: 0.0001)
    }
    ...
    func test_largeAttempt_isCappedAtMaxDelay_evenWithJitter() {
        ...
        for _ in 0..<200 {
            let delay = self.policy.delay(for: 20, retryAfter: nil)
            let seconds = self.durationSeconds(delay)
    ...
    func test_defaultPolicy_hasSpecValues() {
        ...
        XCTAssertEqual(self.durationSeconds(d.initialDelay), 1.0, accuracy: 0.0001)
        XCTAssertEqual(self.durationSeconds(d.maxDelay), 30.0, accuracy: 0.0001)
